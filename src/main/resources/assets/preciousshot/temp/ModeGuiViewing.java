...
public class ModeGuiViewing extends GuiScreen {
	...
	public void initGui() {
		...
		this.twitterText = new GuiTextField(8, this.mc.fontRenderer, this.width/2-100, this.height/2, 200, 20);
		...
		switch(currentPhase) {
			case LIST:
				...
				for(int i=0; i<=numPerPage-1; i++) {
					...
					if(this.mc.fontRenderer.getStringWidth(name) > innerBoxWidth) {
						while(this.mc.fontRenderer.getStringWidth(name+"...") > innerBoxWidth) {
							name = name.substring(0, name.length()-2);
						}
						name = name + "...";
					}
					int strLeft = (int)(((double)innerBoxWidth-(double)this.mc.fontRenderer.getStringWidth(name)) / 2D);
					this.fontRenderer.drawString(name, left+col*boxWidth+(boxWidth-innerBoxWidth)/2+strLeft,
			...
			case EACH:
				...
				String name = resource.file.getName();
				if(this.mc.fontRenderer.getStringWidth(name) > this.width) {
					while(this.mc.fontRenderer.getStringWidth(name+"...") > this.width) {
						name = name.substring(0, name.length()-2);
					}
					name = name + "...";
				}
				int strLeft = (int)(((double)this.width-(double)this.mc.fontRenderer.getStringWidth(name)) / 2D);
				this.fontRenderer.drawString(name, strLeft, top2+(int)scaledHeight+1, 0xFFFFFF);
