...
public class ModeGuiViewing extends Screen {
	...
	public void init() {
		...
		this.eachPrevButton	= new ViewingButton(6,	124+x,	0+y, 16, 16, 32, 96, button -> {
		...
		this.eachNextButton	= new ViewingButton(7,	142+x,	0+y, 16, 16, 48, 96, button -> {
			if(ResourceManager.getSize() == 0) {
				this.currentIndex = 0;
			}
			else {
		...
		this.twitterText = new TextFieldWidget(this.font, this.width/2-100, this.height/2, 200, 20, "");
...
//		GuiUtils.drawRect(0, 0, this.width, this.height, 0x50000000);
		this.fillGradient(0, 0, this.width, this.height, 0x50000000, 0x50000000);
...
//							0, 0, resource.width, resource.height, resource.width, resource.height);
					...
					this.minecraft.fontRenderer.drawString(name, left+col*boxWidth+(boxWidth-innerBoxWidth)/2+strLeft,
...
//						0, 0, resource.width, resource.height, resource.width, resource.height);
				...
				this.minecraft.fontRenderer.drawString(name, strLeft, top2+(int)scaledHeight+1, 0xFFFFFF);
				
				break;
		}

		super.render(mouseX, mouseY, partialTicks);
		this.twitterText.render(mouseX, mouseY, partialTicks);
	...
	private class ViewingButton extends ExtendedButton {
		...
		protected void drawButtonBasic(int mouseX, int mouseY) {
			if(this.visible) {
				...
				GuiUtils.drawTexturedModalRect(this.x, this.y, this.textureX, this.textureY, this.width, this.height, 10);
